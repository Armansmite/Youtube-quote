...
# MoviePy v2 import (fallback to old if needed)
try:
...
# ---------- Duplicate check (YouTube) ----------
def video_already_uploaded(service, title):
    try:
        ...
        for i in range(0, len(video_ids), 50):
            ...
            vid_response = service.videos().list(part="snippet", id=",".join(batch)).execute()
...
# ---------- Pre‑rendered video helpers ----------
def load_videos_processed():
    if not os.path.exists("videos_processed.txt"): return set()
    with open("videos_processed.txt", "r") as f:
        return {line.strip() for line in f.read().splitlines() if line.strip()}

def mark_video_processed(filename):
    with open("videos_processed.txt", "a") as f:
        f.write(f"{filename}\n")
...
def main():
    ...
    config_vals = settings.get("current_config_settings", {})
    total_duration = config_vals.get("total_duration", 7)
    fade_duration = config_vals.get("fade_duration", 2)
    max_quote_len = config_vals.get("max_quote_len", 50)
    min_quote_len = config_vals.get("min_quote_len", 0)

    send_log(f"📊 Settings: total_duration={total_duration}, fade_duration={fade_duration}, "
             f"max_quote_len={max_quote_len}, min_quote_len={min_quote_len}")
    ...
    send_log(f"Already processed quotes: {sorted(processed)}")
    ...
    if os.path.exists("quote.txt"):
        ...
        if to_process:
            ...
            for line_idx, line, data in to_process:
                if max_videos is not None and videos_processed_count >= max_videos:
                    send_log(f"Reached max videos limit ({max_videos}). Stopping quote processing.")
                    break
                ...
                if len(quote) < min_quote_len:
                    send_log(f"Line {line_idx}: quote too short ({len(quote)} chars), marking processed.")
                    mark_processed(line_idx)
                    continue

                candidate_title = f"{quote} – {writer}"[:100]
                if video_already_uploaded(service, candidate_title):
                    send_log(f"Line {line_idx}: already uploaded (found on YouTube). Marking processed.")
                    mark_processed(line_idx)
                    continue
            ...
            send_log("No unprocessed quotes. Skipping quote processing.")
    else:
        send_log("quote.txt not found. Skipping quote processing.")

    # ---------- PRE‑RENDERED VIDEO PROCESSING ----------
    videos_dir = os.path.join("config", "videos")
    if os.path.isdir(videos_dir):
        video_files = sorted(glob.glob(os.path.join(videos_dir, "*.mp4")))
        if video_files:
            send_log(f"📁 Found {len(video_files)} pre‑rendered video(s).")
            service = build("youtube", "v3", credentials=creds)

            # Re‑fetch scheduled times (in case quotes were processed and occupied set changed)
            def get_scheduled_times():
                channel_response = service.channels().list(part="contentDetails", mine=True).execute()
                uploads_id = channel_response["items"][0]["contentDetails"]["relatedPlaylists"]["uploads"]
                video_ids = []
            ...
            occupied = get_scheduled_times()
            send_log(f"Found {len(occupied)} already scheduled slots (for video files).")
            ...
            next_slot = next_free_slot(occupied, slot_tuples)
            send_log(f"Next free slot for pre‑rendered videos: {next_slot.strftime('%Y-%m-%d %H:%M:%S UTC')}")

            videos_processed_set = load_videos_processed()

            for video_file in video_files:
                filename = os.path.basename(video_file)
                if filename in videos_processed_set:
                    continue   # already handled in a previous run

                title = os.path.splitext(filename)[0]  # filename without .mp4
                if video_already_uploaded(service, title):
                    send_log(f"🎬 '{title}' already exists on YouTube. Marking as processed.")
                    mark_video_processed(filename)
                    continue

                try:
                    description = (
                        f"{title}\n\n"
                        f"✨ Topic: Pre‑rendered\n"
                        f"{description_extra}\n"
                        f"🔖 #quotes #motivation #wisdom\n\n"
                        f"🎵 Music from YouTube Audio Library\n"
                        f"📌 Subscribe for daily quotes"
                    )
                    tags = list(set(base_tags + ["pre-rendered"]))
                    ...
                    while response is None:
                        status, response = request_obj.next_chunk()
                    send_log(f"📤 Uploaded '{title}' and scheduled at {publish_str}")
                    ...
                    send_log(f"❌ Failed to upload '{title}': {e}")
        else:
            send_log("No pre‑rendered videos found.")
    else:
        send_log("No config/videos directory. Skipping pre‑rendered videos.")
