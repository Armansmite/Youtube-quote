...
def video_already_uploaded(service, title):
    """Check if a video with the exact title already exists on the channel."""
    try:
...
def main():
    ...
    # Config‑specific settings (merged by dashboard)
    ...
    for line_idx, line, data in to_process:
        ...
        # Create video using the config function (with duration/fade)
        try:
