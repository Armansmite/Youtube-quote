...
# ================================================================
#  SETTINGS DEFINITION – automatically shown in the web dashboard
# ================================================================
SETTINGS_DEF = {
    "total_duration": {
        "default": 7,
...
def create_video(quote, writer, subject, image_path, music_path, output_path,
    ...
    Classic V1 style:
      ...
      - Fade‑in from black
      - Quote text (two lines, 2nd line shorter) with black stroke
      - Writer name below, smaller
      - Audio: first `total_duration` seconds of the music file
    """
    ...
    max_text_width = target_w - 200

    # --- 2. Font loading (Garamond → system serif → default) ---
    font = None
    ...
    # --- 3. Auto‑size helper (binary search) ---
    ...
    # --- 4. Split quote into two lines (2nd line shorter) ---
    words = quote.split()
    if len(words) <= 1:
        lines = [quote]
    else:
    ...
    quote_size = best_size(lines, max_text_width)
    ...
    # Write output (MoviePy v2 – no verbose / logger arguments)
