#!/usr/bin/env python3
"""
automated_youtube_shorts_v1_online.py

V1: 7-second vertical shorts with subject-specific background and music.
Schedules videos at 05:30 UTC and 17:30 UTC (one per slot per day).
Uses YouTube titles to avoid re-uploading already processed quotes.
No local processed.txt needed.
"""
...
# Two daily upload slots (UTC)
SLOT_HOURS = [(5, 30), (17, 30)]
...
# ----------------------------------------------------------------------
# Get all existing video titles (to avoid re-upload)
# ----------------------------------------------------------------------
def get_existing_titles():
    """Return a set of titles of all videos on the channel (public + scheduled)."""
    ...
    for i in range(0, len(video_ids), 50):
        ...
        vid_response = youtube.videos().list(part="snippet", id=",".join(batch)).execute()
        for item in vid_response["items"]:
            titles.add(item["snippet"]["title"])
    return titles

# ----------------------------------------------------------------------
# Get scheduled video times (still needed for slot management)
# ----------------------------------------------------------------------
...
def main():
    ...
    # 1. Fetch existing video titles (so we don't re-upload)
    logger.info("Fetching existing YouTube video titles to avoid duplicates...")
    existing_titles = get_existing_titles()
    logger.info(f"Found {len(existing_titles)} existing videos on the channel.")

    # 2. Fetch already scheduled slots
    logger.info("Fetching existing scheduled slots...")
    ...
    for line_idx, raw_line in enumerate(lines, start=1):
        ...
        if not all(k in data for k in ("qoute", "writer", "subject")):
            logger.error(f"Line {line_idx}: missing keys (qoute, writer, subject). Skipping.")
            continue
        ...
        if len(quote) > MAX_QUOTE_LEN:
            logger.info(f"Line {line_idx}: quote too long ({len(quote)} chars). Skipping.")
            continue

        # Check if this exact title already exists on the channel
        candidate_title = f"{quote} – {writer}"[:100]
        if candidate_title in existing_titles:
            logger.info(f"Line {line_idx}: already uploaded (title exists). Skipping.")
            continue
        ...
        try:
            ...
            # Add this new title to our set so we don't reprocess it in the same run
            existing_titles.add(candidate_title)
            # Reserve the slot and find the next free one
