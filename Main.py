...
Text auto-sizes, wraps into two lines (2nd line shorter), writer smaller.
"""
...
# Preferred font file (place Garamond.ttf next to this script)
...
# Layout (Y positions from top – text is centre‑aligned horizontally)
QUOTE_Y = 700
WRITER_Y = 850

# Writer font size as a fraction of the quote font size
...
def load_font(size):
    ...
    # 3. Pillow default (always works)
    logger.warning("No serif font found, using Pillow default.")
    return ImageFont.load_default()

# ----------------------------------------------------------------------
# Quote wrapping into two lines with 2nd line shorter
# ----------------------------------------------------------------------
def split_quote_two_lines(quote):
    """
    Split quote into two lines (at most two) such that the second line is
    shorter than the first. Returns a list of 1 or 2 strings.
    """
    words = quote.split()
    if len(words) <= 1:
        return [quote]

    # Try breaking at spaces, starting from the middle and moving leftwards
    n = len(words)
    best_split = None
    # We want first part longer than second part in terms of number of words
    for i in range(n - 1, 0, -1):          # i = number of words in first line
        first = " ".join(words[:i])
        second = " ".join(words[i:])
        if len(first) >= len(second):      # first line at least as long as second
            # Prefer a split where first is strictly longer, but accept equal if needed
            if best_split is None or (len(first) > len(second) and len(best_split[0]) == len(best_split[1])):
                best_split = (first, second)
            if len(first) > len(second):
                break                       # found a clearly longer first part
    if best_split is None:
        # If no split satisfies length condition, just keep as one line
        return [quote]
    return list(best_split)

# ----------------------------------------------------------------------
# Determine optimal font size for multi‑line text
# ----------------------------------------------------------------------
def best_font_size(lines, max_width):
    """
    Return the largest integer font size such that each line in `lines`
    fits within max_width using the available font.
    """
    # Try to use a truetype font for width measurement
    font_path = None
    if os.path.isfile(FONT_FILE):
        font_path = FONT_FILE
    else:
...
# ----------------------------------------------------------------------
# Build composite frame (new layout with two‑line quote)
# ----------------------------------------------------------------------
def build_text_frame(image, quote, writer):
    """Return PIL image with quote (1 or 2 lines, 2nd line shorter) and writer."""
    img = image.copy()
    draw = ImageDraw.Draw(img)
    max_width = VIDEO_SIZE[0] - 2 * MARGIN

    # Split quote into up to two lines
    lines = split_quote_two_lines(quote)

    # Calculate optimal font size for the longest line
    quote_size = best_font_size(lines, max_width)
    quote_font = load_font(quote_size)

    # Writer font size (smaller)
    writer_size = max(int(quote_size * WRITER_FONT_RATIO), 12)
    writer_font = load_font(writer_size)

    # Draw each quote line centred horizontally
    line_height = quote_font.getbbox("Ag")[3] - quote_font.getbbox("Ag")[1]
    total_height = line_height * len(lines) + (len(lines) - 1) * 10   # 10px gap between lines
    ...
    # Draw writer name below the quote block
    ...
    writer_y = start_y + total_height + 30   # 30px below the last quote line
...
# ----------------------------------------------------------------------
# Video creation (unchanged except calling new build_text_frame)
# ----------------------------------------------------------------------
def create_video(quote, writer, subject, image_path, music_path, output_path):
    ...
    # Build frame with two‑line quote + smaller writer
