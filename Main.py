...
# ---------- 2. Imports ----------
import os, json, time, requests, pickle, random, re, glob, warnings, traceback, threading
import numpy as np
...
def send_heartbeat():
    """Tell dashboard that we are alive."""
    try:
        requests.post(DASHBOARD_URL + "api/connect", json={"connected": True}, timeout=5)
    except:
        pass

def stop_heartbeat():
    try:
        requests.post(DASHBOARD_URL + "api/connect", json={"connected": False}, timeout=5)
    except:
        pass

# ---------- 7. V1 Helper Functions (same as before) ----------
def load_processed():
...
def main():
    # Announce connection
    send_heartbeat()
    send_log("🚀 Colab connected to dashboard.")

    # Start a heartbeat thread that sends pings every 30 seconds
    def heartbeat_loop():
        while bot_running:
            time.sleep(30)
            if bot_running:
    ...
    try:
        ...
        # 3. Fetch settings from dashboard (max_videos included)
        ...
        max_videos = settings.get("max_videos", 0)
        if max_videos == 0:
        ...
        send_log(f"Settings: duration={total_duration}s, fade={fade_duration}s, max videos={max_videos or 'unlimited'}")
