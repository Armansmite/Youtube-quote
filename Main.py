...
# ----------------------------------------------------------------------
# Helper functions (same as V1, untouched)
# ----------------------------------------------------------------------
def load_processed():
...
# In-memory state (per session – simplified: one global for Colab)
global_state = {
    "settings": DEFAULT_SETTINGS.copy(),
    "credentials": None,
    "flow": None,  # OAuth flow object (needed for auth)
    "running": False,
    "log": []
}
...
def authenticate():
    ...
    try:
        global_state["flow"].fetch_token(code=code)
...
def bot_process():
    try:
        ...
        # get scheduled times (reuse old function but with service)
...
# ----------------------------------------------------------------------
# HTML Frontend (embedded as a multi‑line string)
# ----------------------------------------------------------------------
HTML_CONTENT = r"""
<!DOCTYPE html>
<html lang="en">
...
def start_ngrok():
    ...
    ngrok.kill()
    # Set auth token if you have one (optional)
...
if __name__ == "__main__":
    # Create a dummy index.html to avoid 404 on root (we redirect to frontend)
