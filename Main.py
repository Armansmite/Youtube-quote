...
# ----------------------------------------------------------------------
# YOUR COLAB NOTEBOOK URL – change this
# ----------------------------------------------------------------------
COLAB_URL = "https://colab.research.google.com/drive/your-notebook-id"
...
# We'll write them to disk when the script starts.
...
@app.route("/icon.png")
def serve_icon():
    # A simple purple/teal icon (base64 encoded 192x192 PNG)
    # This is just a placeholder – you can replace it with a real icon
    icon = "iVBORw0KGgoAAAANSUhEUgAAAMAAAADACAYAAABS3Gw..." # truncated
...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open('shorts-v1').then(cache => {
      return cache.addAll(['/app.html', '/manifest.json', '/icon.png']);
    })
  );
});
