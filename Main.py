...
generates a thumbnail, and schedules them at 05:30 UTC daily.
Uses Pillow for text – NO ImageMagick required.
Reads authorization code from auth_code.txt file (Colab-friendly).

Stops immediately if a real upload error occurs (e.g. quota exceeded).
Thumbnail errors are non‑fatal (just a warning).
"""
...
# ----------------------------------------------------------------------
# Upload video – returns video_id on success
# Thumbnail failure is non‑fatal (just a warning)
# Any other error (e.g. upload limit) is raised and must be handled by caller
# ----------------------------------------------------------------------
def upload_video(video_path, thumbnail_path, title, description, tags, category_id, publish_at=None):
    ...
    # Upload the video file (raises exception if quota exceeded, network error, etc.)
    ...
    # Thumbnail – optional, failure is not an error
    if thumbnail_path and os.path.exists(thumbnail_path):
        try:
            ...
            # Do NOT re-raise – the upload itself was successful
...
def main():
    ...
    for line_idx, line, data in to_process:
        ...
        try:
            ...
            # Upload succeeded – mark line as processed and reserve this slot
            mark_processed(line_idx)
            occupied.add(next_slot)
            next_slot = next_free_530(occupied)
            logger.info(f"Line {line_idx}: successfully scheduled at {next_slot.strftime('%Y-%m-%d %H:%M:%S UTC')}")

        except Exception as e:
            # Real upload error (quota, network, etc.) – stop everything
