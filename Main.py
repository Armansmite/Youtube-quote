...
# ----------------------------------------------------------------------
# Global ngrok URL (set after tunnel starts)
# ----------------------------------------------------------------------
NGROK_URL = None
...
def get_ngrok_url():
    return jsonify({"url": NGROK_URL or ""})
...
# ----------------------------------------------------------------------
HTML_CONTENT = r"""..."""  # (same as before)

# ----------------------------------------------------------------------
# PWA static files (with automatic ngrok URL detection)
# ----------------------------------------------------------------------
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .install-btn { position: absolute; top: 10px; right: 10px; background: #00d4aa; color: #000; padding: 8px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; cursor: pointer; border: none; display: none; }
  </style>
</head>
<body>
  ...
  <div id="panel-dashboard" class="panel">
    <div style="display:flex; gap:10px; align-items:center;">
      <input type="text" id="ngrokUrl" placeholder="Auto‑detecting ngrok URL..." style="flex:1;">
  ...
  <script>
    ...
    window.addEventListener('load', () => {
      ...
      // Try to get the ngrok URL from the backend
      fetch('/api/ngrok-url')
        .then(r => r.json())
        .then(data => {
          if (data.url) {
            document.getElementById('ngrokUrl').value = data.url;
            // Optionally save in localStorage
            localStorage.setItem('ngrokUrl', data.url);
          } else {
            // Fallback to localStorage
            const saved = localStorage.getItem('ngrokUrl');
            if (saved) document.getElementById('ngrokUrl').value = saved;
          }
        })
        .catch(() => {
          const saved = localStorage.getItem('ngrokUrl');
    ...
    function loadDashboard() {
      ...
      if (!url) return alert('No ngrok URL available.');
