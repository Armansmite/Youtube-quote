...
DEFAULT_SETTINGS = {
    ...
    "fade_duration": 2,
    "max_quote_len": 50,
    "slots": ["05:30", "11:30", "17:30", "23:30"],   # list of "HH:MM" strings
...
def run_bot():
    ...
    max_videos = data.get("max_videos", None)  # None = unlimited
    global_state["log"] = []
...
def bot_process(max_videos=None):
    try:
        ...
        if not creds or not creds.valid:
            add_log("ERROR: Not authenticated.")
            return
...
# ----------------------------------------------------------------------
# HTML Frontend (beautiful glassmorphism design)
# ----------------------------------------------------------------------
HTML_CONTENT = r"""
<!DOCTYPE html>
<html lang="en">
...
<div class="container">
  ...
  <div id="panel-settings" class="panel active">
    ...
    <div class="card">
      ...
      <button class="add-btn" onclick="addSlot()">+ Add Slot</button>
    </div>
  ...
  <div id="panel-auth" class="panel">
    <div class="card">
      ...
      <button onclick="document.getElementById('client_secret_file').click()" class="mt-1">📁 Choose File</button>
    ...
    <div class="card">
      ...
      <div class="inline mt-1">
        ...
        <button onclick="copyAuthUrl()" style="width:auto;">📋 Copy</button>
      </div>
      <div class="inline mt-1">
        <input type="text" id="auth_code" placeholder="Paste the full redirect URL or code...">
        <button onclick="authenticate()" style="width:auto;">🔑 Authenticate</button>
      </div>
  ...
  <div id="panel-run" class="panel">
    ...
    <button onclick="startBot()" style="margin:20px 0;">▶️ Start Processing</button>
  ...
  function renderSlots() {
    ...
    container.innerHTML = timeSlots.map((slot, index) => `
      <div class="time-slot-row">
        ...
        <button onclick="removeSlot(${index})" style="width:auto;">✕</button>
      </div>
    `).join('');
  }
