...
generates a thumbnail from the middle frame, and **schedules** them
for publication on the next available day at 05:30 UTC.
...
# ----------------------------------------------------------------------
# Build composite frame (returns PIL Image)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Get upcoming scheduled video times (private, future publishAt)
# ----------------------------------------------------------------------
def get_upcoming_scheduled_times():
    """Returns a set of datetime objects for videos already scheduled in the future."""
    ...
    # 3. Get status and publishAt for all videos
    scheduled_times = set()
    ...
    for i in range(0, len(video_ids), 50):
        ...
        for item in vid_response["items"]:
            ...
            if status.get("privacyStatus") == "private" and "publishAt" in status:
                # publishAt is in ISO 8601 format with timezone, e.g. "2026-06-05T05:30:00Z"
...
# ----------------------------------------------------------------------
# Find next free 05:30 UTC slot
# ----------------------------------------------------------------------
def next_free_530(occupied_set):
    """Return the next datetime at 05:30 UTC that is not in occupied_set."""
    now = datetime.utcnow()
...
# ----------------------------------------------------------------------
# Upload video (with optional scheduling) and set thumbnail
# ----------------------------------------------------------------------
def upload_video(video_path, thumbnail_path, title, description, tags, category_id, publish_at=None):
    """
    Upload a video and optionally schedule it for a future publish time.
    If publish_at is provided, video is uploaded as private and publishAt set.
    Otherwise it's public immediately.
    ...
    if publish_at:
        # Format datetime as RFC 3339 string (e.g., "2026-06-05T05:30:00Z")
...
# ----------------------------------------------------------------------
# Video creation (saves thumbnail too)
# ----------------------------------------------------------------------
...
def main():
    ...
    # Get existing scheduled times (future private videos)
    ...
    for line_idx, line, data in to_process:
        ...
        # Success → mark as processed, advance to next day slot
        mark_processed(line_idx)
        occupied.add(next_slot)
        next_slot = next_free_530(occupied)   # find the next free day
